...
class PapersController extends Controller {
    ...
    */
    ...
    public function get_from_corrected($pid, $sid, $tid){
        ...
        ->where([
          ['teacher_record.teacher_id' , $tid],
          ['student_record.student_id' , $sid],
          ['totalpq.paper_id'   , $pid]
        ])
        ->select('totalpq.question_id', 'teacher_record.has_correct','teacher_record.grade', 'student_record.description')
        ->get();
    ...
    public function uncorrect_paper_teacher(Request $req){
       ...
       $papers = PaperTool::join()
                ->where([
                  ['teacher_record.teacher_id', $teacher_id],
                  ['teacher_record.has_correct', 0]
                ])
                ->select('teacher_record.teacher_id', 'student_record.student_id', 'totalpq.paper_id', 'paper.title')
                ->groupBy('teacher_record.teacher_id', 'student_record.student_id', 'totalpq.paper_id')
                ->havingRaw('COUNT(*) > 0')
                ->paginate(self::PAGE);
    ...
    public function correct_paper_teacher(Request $req){
      ...
      $papers = PaperTool::join()
               ...
               ->select('teacher_record.teacher_id', 'student_record.student_id', 'totalpq.paper_id',
